...
def tree2mip(tree, mip_model:grb.Model, model_vars, return_value ='class_idx_max', tree_idx =0):
    ...
    def recurse(node):
        if tree_.feature[node] != _tree.TREE_UNDEFINED:
            ...
            mip_model.addConstr((which_branch == 1) >> (model_vars['counterfactual'][name]['var'] <= threshold - 1e-3))
            mip_model.addConstr((which_branch == 0) >> (model_vars['counterfactual'][name]['var'] >= threshold + 1e-3))
...
def lr2mip(lr_model, mip_model:grb.Model, model_vars):
   ...
   mip_model.addConstr(
       ...
       grb.quicksum(
           model_vars['counterfactual'][key]['var'] * float(lr_model.coef_[0][idx])
           for idx, key in enumerate(model_vars['counterfactual'].keys())
       )
   )

   mip_model.addConstr((model_vars['output']['y']['var'] == 1) >> (logit_out >= 0.0))
   mip_model.addConstr((model_vars['output']['y']['var'] == 0) >> (logit_out <= 0.0))
   # mip_model.addConstr(logit_out <= 0)
